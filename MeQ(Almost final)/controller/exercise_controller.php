...
if(isset($_GET['te']))
{
   ...
   echo '<div Class=ImageExercise>';
      if(!empty($image)) echo '<img onclick="image(this)" style="width: 100%;height: 100%; max-height: 100px;max-width: 90px; border:0.5px solid black;" src="../view/images/'.$image[0]['IMG_NAME'].'">';
      else
      {
          ?>
           <script>
  ...
  <?php
   ...
   echo '<textarea name="TitleContent" readonly="readonly"; style="width: 395px; height: 100%; text-align: center; resize: none; border:1px solid black;">';
   ...
   echo '<textarea name="UpdateExerciseContent" readonly="readonly" style="width:500px; resize: none ;border:1px solid black;">';
    ...
    foreach ($replies as $reply) {
        if ($reply['EXERCISE_ID'] == $exercise[0]['ID']) {
            ...
            if ((($_SESSION['Username'] == $user[0]) && (isset($_SESSION['Username'])))||(isset($_SESSION['IsAdmin']))) {
                echo '<form method="post">';
                echo '<textarea name="ReplyText"  style="width: 98%;height: 95%; resize: none;overflow-y: hidden; border:1px solid black;">';
                echo $reply['CONTENT'];
                echo '</textarea>';
        ...
        //    echo '</textarea>';
            ...
            if (!empty($reply['IMAGE_NAME']))
                echo '<img onclick="imageReply(this)" style="max-width:50px;max-height:50px; border:0.5px solid black;"src="../view/images/' . $reply['IMAGE_NAME'] . '">';
            echo $reply['IMG_NAME'];
            echo '</div>';
            echo '</div>';
