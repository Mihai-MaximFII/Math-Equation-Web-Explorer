        ...
        if($log['Category']==1){
            ...
            if($log['Category']==2){
                ...
                if($log['Category']==3) {
                    ...
                    if($log['Category']==4){
                        if($exists==1) {
                            echo '<p>' . $log['Frm'] . ' updated his thread :' . "<a target='_top' href='../home.php?t=" . $log['Title'] . "'" . ">" . $log['Title'] . "</a>" . '<br>' . $log['updated_at'] . '</p>';
                        }}
                    else
                        if($log['Category']==5){
                            if($exists==1) {
                            ...
                            if($log['Category']==6){
                                ...
                                if($log['Category']==7){
                                    ...
                                    if($log['Category']==8){
                                        if($exists==1) {
                                            echo '<p>' . $log['Frm'] . ' updated his exercise: ' . "<a target='_top' href='../view/ExerciseView.php?te=" . $log['Title'] . "'" . ">" . $log['Title'] . "</a>" . '<br>' . $log['updated_at'] . '</p>';
                                        }}
                                    else
                                        if($log['Category']==9) {
                                            echo '<p>' . $log['Frm'] . ' deleted his exercise:  ' . $log['Title'] . '<br>' . $log['updated_at'] . '</p>';
                                        }
                                        else
                                            if($log['Category']==10){
                                                if($exists==1) {
