...
<xsl:template match="/Threads">
  ...
  <body>
      <UL>
          ...
          <li><a href="../home.php?LimitOn=true&amp;PressMe=">Only 3 replies</a></li>
          <li><a href="../home.php?LimitOff=true&amp;PressMe=">All replies</a></li>
          <li><a href="../home.php?NoReplies=true&amp;PressMe=">No replies</a></li>
      </UL>
      <script>
          function iniFrame() {
    ...
    <xsl:for-each select="Thread">
        ...
        <div class="blended_grid">
            ...
            <div class="Left">
                ...
                <img style="float:right; border:2px solid black;"><xsl:attribute name="src">images/<xsl:value-of select='Image' /></xsl:attribute></img>
                    </xsl:if>
                </xsl:if>
            </div>
            <div class="Top1">
                ...
                <xsl:if test = "Image !=''">
                <h5><a style="color:black;"><xsl:attribute name="href">images/<xsl:value-of select="Image"/></xsl:attribute><xsl:value-of select="Image"/></a></h5>
                </xsl:if>
            </div>
            ...
            <div class="blended_grid2">
                <div contenteditable="true" class="ReplyTop"  >

                    <form action="../controller/manage_reply_controller.php" method="POST">
                        <xsl:choose>
                        <xsl:when test="ReplyAdmin > 0">
                    <textarea Class="Text ReplyText" name="ReplyRequestText" maxlength="2000" ID="ReplyForm"  ><xsl:value-of select="ReplyContent"/></textarea>
                        </xsl:when>
                        <xsl:when test="ReplyAdmin = -1">
                            <textarea class="Text ReplyText" readonly="readonly" name="ReplyRequestText" maxlength="2000" ID="ReplyForm"  ><xsl:value-of select="ReplyContent"/></textarea>
                        </xsl:when>
                        </xsl:choose>
                ...
                <div class="ReplyRight1">
                    ...
                    <h5><a style="color:black;"><xsl:attribute name="href">images/<xsl:value-of select="ReplyImage"/></xsl:attribute><xsl:value-of select='ReplyImage' /></a></h5>
                    </xsl:if>
            ...
            <div class="Reply">
                <form method="POST" enctype="multipart/form-data" action="../controller/reply_controller.php"  >
                    <textarea  ID="ReplyForm" name="CreateReplyRequestText" rows="5" maxlength="5000" cols="50"  style="border:solid 1px black; border-radius:20px;padding:10px;" ></textarea>
