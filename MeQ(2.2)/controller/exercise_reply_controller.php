        ...
        if(isset($_FILES['CreateExerciseReplyRequestFile']))
        {
            ...
            if(in_array($filextlow,$allowed))
            {
                if($error===0){
                    if($size<1000000)
                    {
                        ...
                        if(isset($_SESSION['Username'])) {
                            $title=$users->getTitleById($_POST['CreateExerciseReplySubmit']);
                            $logs->insertLog($_SESSION['Username'],$title[0]['Title'],7);
                            ...
                            $title=$users->getTitleById($_POST['CreateExerciseReplySubmit']);
                            $logs->insertLog($_SESSION['Anon'],$title[0]['Title'],7);
                            $replies->insertExerciseReplyAnon(0, $_POST['CreateExerciseReplySubmit'], $_POST['CreateExerciseReplyRequestText'], $filenew, $_SESSION['Anon']);
                            $relevance=$exercises->getRelevance( $_POST['CreateExerciseReplySubmit']);
                            $exercises->setRelevance( $_POST['CreateExerciseReplySubmit'],$relevance[0][0]);
                        }
            ...
            {$rows=$exercises->getExerciseIdByUser($_SESSION['Anon']);
                ...
                if(isset($_SESSION['Username'])) {
                    ...
                    $title=$users->getTitleById($_POST['CreateExerciseReplySubmit']);
                    $logs->insertLog($_SESSION['Anon'],$title[0]['Title'],7);
                    $replies->insertExerciseReplyAnon(0, $_POST['CreateExerciseReplySubmit'], $_POST['CreateExerciseReplyRequestText'], "", $_SESSION['Anon']);
                    $relevance=$exercises->getRelevance( $_POST['CreateExerciseReplySubmit']);
                    $exercises->setRelevance( $_POST['CreateExerciseReplySubmit'],$relevance[0][0]);
                }
                sleep(3);
